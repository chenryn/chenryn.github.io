...
然后进 Visualize 页，添加一个 pie chart。选择 aggregation 类型为 terms。选择字段为 `slow.1`(如果采用了类 logstash 的template，这里应该用 `slow.1.raw` 确保函数不会被分词)。然后点击 `split bucket`，继续添加 aggregation，这次字段为 `slow.2`。以此类推，假设我们一直添加到了 `slow.4`。

好了，页面右侧出现了最终的效果：

![split bucket pie chart](/images/uploads/k4-split-bucket-pie.jpg)
